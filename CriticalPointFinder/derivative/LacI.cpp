...
void LacIgraph::getCriticalPt(double limit)
{
    ...
    while(!file.eof())
    {
      //pull in from each line of the file, the current LacI value, 
      //the rate at that value, and the instantaneous Change in reaction rate
        file >> current >> currentRate >> CinRR;


        if(abs(CinRR) > (limit))
        {
            //print current and currentRate to new file,
            //the second value doesn't matter, it is only printed
            //so that the two values line up properlly when graphed
