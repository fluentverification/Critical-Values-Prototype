...
double degradationRange(int lowerInt, int upperInt);
...
for(int j = 0; j <= (InitialValue / epsilon); j++)
{
	...
	//first line
	if (j == 0){
		PrismModel << "	[] TetR=" << j * epsilon << " -> " 
		<< repressionRange( j * epsilon, (j+1) * epsilon ) << " : " << "(TetR'=TetR+" << epsilon << ");" << std::endl;
	//last line
	}else if(j == InitialValue / epsilon){

		PrismModel << "	[] TetR=" << j * epsilon << " -> kd / (" 
		<< degradationRange( j * epsilon, (j+1) * epsilon ) << ") : " << "(TetR'=TetR-" << epsilon << ");" << std::endl;
	...
	//main bulk
		...
		PrismModel << "	[] TetR=" << j * epsilon << " -> " 
		//repression part
		<< repressionRange( j * epsilon, (j+1) * epsilon ) << " : " << "(TetR'=TetR+" << epsilon << ")"
		//degradePart
		<< " + kd / ("<< degradationRange( j * epsilon, (j+1) * epsilon ) << ") : " << "(TetR'=TetR-" << epsilon << ");" << std::endl;
